...
A static site generated by Python. No Node, no framework, no runtime, no
database. `build.py` renders 16 HTML pages and resizes the photographs;
the output in `dist/` is plain files.

| Requirement | Value |
|---|---|
| Python | 3.8 or newer (verified by parsing every source file) |
| Dependency | Pillow >= 12.3.0 — the only one |
| Build | `pip install -r requirements.txt && python build.py` |
| Output | `dist/` |
| Build time | ~10s cold, ~2s warm (image variants are cached) |
...
`verify.py` must print `all checks passed` and exit 0. It checks every
...
```bash
rm -rf dist && python build.py && mv dist dist-a
python build.py && diff -r dist-a dist && echo OK
rm -rf dist-a
```
...
build, `verify.py`, then a second build compared against the first.
...
3. Vercel reads `vercel.json` from the repo root, which already sets:
   ...
   - `trailingSlash: true` — the site uses `/about/` style URLs
   - security headers and cache policy (see step 6)
4. Deploy.

**If the build fails because Python or pip is unavailable** in Vercel's
build image, do not fight it. Switch to building in CI and shipping the
output:

```yaml
- run: pip install -r requirements.txt && python build.py && python verify.py
- run: npx vercel deploy --prebuilt --prod --token=${{ secrets.VERCEL_TOKEN }}
```

The site has no server-side component, so where it is built does not
matter.
...
`SITE` at the top of `build.py` is `https://goldcoastbld.com`. It feeds the
...
Content lives in `editorial.py`, `build.py` and `content.json`; photographs
in `src/assets/`. Never edit anything in `dist/` — it is regenerated.
